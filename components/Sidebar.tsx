      ...
      <div className="p-6 border-b ☐ border-gray-700">
        <div className="sm: hidden flex justify-end mb-0">
          <button onClick={() => setSidebarOpen(false)}
            className="p-2 ☐ hover:bg-gray-700 rounded-lg transition-colors"><X className="w-5 h-5 text-gray-300" /></button>
        </div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 ☐ bg-blue-600 justify-between rounded-lg">
          ...
          >
            {showAllUsers ? <X className="w-4 h-4" /> : <Plus className="w-4
h-4" />}
          </button>
        </div>
      </div>
...
border-gray-700 text-white placeholder-gray-400 rounded-full"
                value={searchQuery}
            ...
            <div className="space-y-2 overflow-y-auto h-full pb-4 ">
              {
                users?.filter((u) => u._id !== loggedInUser?._id && u.name.
                  toLowerCase().includes(searchQuery.toLocaleLowerCase())).map
                  ((u) => (
            ...
            chats && chats.length > 0 ? <div className='space-y-2 overflow-y-auto h-full pb-4'>
              {chats.map((chat) => {
                ...
                return <button key={chat.chat._id} onClick={() => {
                  ...
                  className={`w-full text-left p-4 rounded-lg  transition-colors rounded-lg ${isSelected ? "bg-gray-600 border-gray-500" : "border-gray-700 hover:border-gray-600"}`}
                >
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div className="w-12 h-12 rounded-full ☐ bg-gray-700 flex
items-center justify-center">
                        <UserCircle className="w-7 h-7 Itext-gray-300" />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {
                          unseenCount > 0 && (
                            <div className="bg-red-600 text-white text-xs font-bold rounded-full min-w-[22px] h-5.5 flex
                        ...
                        latestMessage && <div className="flex items-center gap-2">
                          {
                            isSentByMe ? <CornerUpLeft size={14}
                              ...
                              <CornerDownRight size={14} className="text-green-400text-shrink-0" />
                            )
                          }
